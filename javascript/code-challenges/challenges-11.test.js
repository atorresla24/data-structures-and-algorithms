...
function transformToLis(obj){
  //let arr = Object.keys(obj).map((key) => [obj[key], Number(key));
  //return arr;
}
...
const totalSum = (input) => {
  let initial = 0;
  let row = input.map(sum => sum.reduce((last, next) => last + next, initial));
  return row.map(value => value.reduce((last, next) => last + next, initial));
};
