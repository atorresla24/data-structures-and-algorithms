...
const toLastNames = people => {
  const toLastNames = people.map((name) => name.firstName + ' ' + name.lastName);
  return toLastNames;
};
...
const addValues = (arr) => {
  const initial = 0;
  return arr.reduce((last, next) => last + next, initial);
};
...
const addPurchases = (arr) => {
  const initial = 0;
  return arr.reduce((last, next) => last + next.purchasePrice, initial);
};
...
const countNumberOfElements = (arr) => {
  return arr.reduce((next) => next + 1, 0);
};
...
const reversedString = (str) => {
  return str.split('').reduce((value, curr) => curr + value, '');
};
